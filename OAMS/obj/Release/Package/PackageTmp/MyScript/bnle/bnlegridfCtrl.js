...
(function () {
    ...
    function ($scope, $timeout, $http, $uibModal, $document, appSettings, loginservice, userProfile) {
        ...
        function getalldatafilter() {
            ...
            if (data.isLoggedIn) {
                var resp = loginservice.postdata("api/funcbase/getalldatafilter", $.param({ valstring1: $scope.funcid }));
                resp.then(function (response) {
                    if (response.data != null) {
                        $scope.viewmode = response.data.viewmode;
                        $scope.formname = response.data.formname;
                        $scope.widthfilter = response.data.widthfilter;
                        $scope.widthgrid = response.data.widthgrid;
                        $scope.datafilter = response.data.datafilter;
                        $scope.tablename = response.data.tablename;
                        $scope.filterkey = "";
                        
                        var t = userProfile.getsaveitem($scope.tablename + "f");
                        if (t != null) {
                            $scope.filterkey = t;
                        }
                        else if ($scope.datafilter.length > 0) {
                            $scope.filterkey = $scope.datafilter[0].CODE;
                            userProfile.saveitem($scope.tablename + "f", $scope.filterkey);
                        }
        ...
        function getdataItems() {
            var data = userProfile.getProfile();
            if (data.isLoggedIn) {
                var resp = loginservice.postdata("api/funcbase/getalldatapagef", $.param({ valint1: $scope.CurrentPage, valstring1: $scope.funcid, valstring2: $scope.filterkey }));
                resp.then(function (response) {
                    if (response.data != null) {
                        debugger
